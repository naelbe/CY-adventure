...
int choiceSA1_2(struct ch target) {
  int i = 0;
  ...
  if (i == 1) {
    ...
    scanf("%s", filename);
    saveGame(&target, 912, filename);
    return 999;
  }
}

int eventSA1_2(struct ch target) {
  ...
  type("Vous obtenez : Epee du soleil levant !\n");
  ...
  return choiceSA1_2(target);
}

int descSA1_2(struct ch target) {
  type("Vous poursuivez votre route, bien que vos compagnons ne conaissent pas Rakai, ils peuvent vous etre utile.\n");
