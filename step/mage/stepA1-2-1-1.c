...
int choiceSA1_2_1_1(struct ch target) {
  int i = 0;
  type("Bravo.\n");
  ...
  if (i == 1) {
    ...
    scanf("%s", filename);
    saveGame(&target, 91211, filename);
    return 999;
  }
}
...
int eventSA1_2_1_1(struct ch target, struct mt monsterT) {
...
int descSA1_2_1_1(struct ch target) {
  struct mt monsterT = constructMonsterSA1_5();
  type("Pour une fois, vous avez fait la bonne decision.\nLa puissance est toujours interessante, mais a quoi sert elle si vous ne pouvez pas trouver de combats ?\n");
  ...
  printf("\033[2J\033[1;1H");
  return eventSA1_2_1_1(target, monsterT);
}
